...
func FindMimeTypeByExt(ext string, defaultExt string, defaultMimeType string) (mimeDetails []string) {
	ext = filepath.Ext(ext)
	if ext == "" {
		ext = defaultExt
	}
	foundMime := make(chan bool)
	defer close(foundMime)
	foundExt := make(chan bool)
	defer close(foundExt)

	go func() {
		rawLineBytes := make([]byte, 1000)
		rawLineBytesIndex := 0
		for _, b := range []byte(rawMimeTypes) {
			switch b {
			case 10:
				if rawLineBytesIndex > 0 {
					go func() {
						mimeSectionsLine := string(rawLineBytes[0:rawLineBytesIndex])
						if strings.Contains(mimeSectionsLine, "\t"+ext+"\t") {
							mimeSections := strings.Split(mimeSectionsLine, "\t")
							if len(mimeSections) == 4 {
								if mimeSections[2] == ext {
									mimeDetails = append(mimeDetails, mimeSections[1], mimeSections[2], mimeSections[0], mimeSections[3])
									foundExt <- true
									return
								}
							}
			...
			case 13:
				break
			default:
				if rawLineBytesIndex < len(rawLineBytes) {
					rawLineBytes[rawLineBytesIndex] = b
					rawLineBytesIndex++
				}
			}
		}
	...
	mimeDetails = append(mimeDetails, defaultMimeType, ext, "Unknown Type", "")
	return mimeDetails
}

//compiled from https://www.freeformatter.com/mime-types-list.html#mime-types-list
const rawMimeTypes = `3D Crossword Plugin	application/vnd.hzn-3d-crossword	.x3d	IANA: 3D Crossword Plugin
